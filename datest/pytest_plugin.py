...
from .discovery import DanaTestDiscovery
from .executor import DanaTestExecutor, DanaTestResult
    ...
    def runtest(self):
        ...
        if not self.result.success:
            ...
            for assertion in self.result.failed_assertions:
                failure_msgs.append(
                    f"Line {assertion.line_number}: {assertion.message}"
                )
            ...
            if hasattr(report, "sections"):
                ...
                if item.result.output:
                    report.sections.append(
                        ("Dana Output", item.result.output)
                    )
